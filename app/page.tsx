...
const details: Record<string, any> = {
  supervisor: {
    ...
    jobdesk: ['Menetapkan target dan prioritas divisi','Memimpin WRM dan monthly review','Mengambil keputusan atas gap revenue dan issue lintas fungsi','Coaching seluruh PIC dan Key Account Leader','Mengawal Marketplace Malaysia bersama agency','Menjaga KPI, action tracker, risk register, dan roadmap'],
  ...
  keyaccount: {
    purpose: 'Memimpin strategi, pengembangan, dan performa Seller Community serta memastikan Sales Relation mengeksekusi account plan secara konsisten.',
    jobdesk: ['Menyusun segmentasi dan strategi seller','Menetapkan account plan seller prioritas','Mereview performa PO, sell-out, retention, dan seller health','Coaching Sales Relation / Account Executive','Menyusun recovery plan seller merah','Mengawal program, NPD, mentoring, dan pengembangan tier','Mengeskalasi kebutuhan support kepada Supervisor'],
    kpi: ['PO dan sell-out Seller Community','Seller retention ≥ 95%','Seller recovery plan completion ≥ 90%','Program dan NPD adoption sesuai target','Kualitas coaching dan action plan'],
    monthly: ['Review seluruh portofolio seller','Tentukan growth, maintenance, dan recovery account','Susun mentoring dan capability plan','Review program, NPD, retention, dan tier movement'],
    weekly: ['Review strategic seller dan at-risk seller','Coaching Sales Relation','Audit kualitas follow-up dan action plan','Tentukan intervensi seller','Review readiness program dan NPD'],
    daily: ['Review issue seller prioritas','Memberikan direction account strategy','Coaching kasus kompleks','Koordinasi kebutuhan seller dengan Operations dan Supervisor','Memastikan follow-up penting ditutup']
  },
  relation: {
    purpose: 'Menjadi PIC utama Seller Community untuk hubungan, target, action plan, program, dan kesiapan NPD di bawah arahan Key Account Leader.',
    jobdesk: ['Follow-up seller sesuai segmentasi','Mengawal target PO dan sell-out','Mengumpulkan laporan sell-out, loyalty, dan stok','Mengkomunikasikan program dan NPD','Mengidentifikasi seller berisiko','Menjalankan account plan dari Key Account Leader','Mengeskalasi kebutuhan support'],
    kpi: ['PO seller achievement','Reporting compliance ≥ 95%','Seller retention ≥ 95%','NPD readiness seller prioritas 100%'],
    monthly: ['Kawal laporan seller','Review seller health bersama Key Account Leader','Jalankan kebutuhan support','Evaluasi program dan NPD'],
    weekly: ['Seller weekly pulse','Follow-up gap PO','Recovery seller merah','Program dan NPD','Seller issue closure'],
    daily: ['Cek seller prioritas','Follow-up seller','Catat hasil dan next action','Follow-up PO, laporan, program, NPD','Eskalasi issue ke Key Account Leader']
  },
  acquisition: {
  ...
  operations: {
    purpose: 'Memastikan PO dapat dipenuhi melalui validasi, alokasi stok, koordinasi produksi, koordinasi ekspedisi Malaysia, delivery, dan outstanding control.',
    jobdesk: ['Memvalidasi PO','Mencocokkan PO dengan stok dan WIP','Membuat shortage analysis','Koordinasi Produksi, SCM, dan Warehouse','Menentukan fulfillment dan ETA','Menyusun delivery plan dan outstanding aging','Menjadi PIC utama koordinasi forwarder atau ekspedisi pengiriman Malaysia','Meminta dan membandingkan quotation freight, trucking, THC, clearance, dan biaya terkait','Mengkoordinasikan jadwal pickup, stuffing, dokumen shipment, tracking, customs, dan ETA','Mengeskalasi perubahan biaya, risiko customs, dan keterlambatan kepada Supervisor'],
    kpi: ['Fulfillment rate ≥ 95%','On-time delivery ≥ 95%','Schedule accuracy ≥ 90%','PO SLA compliance ≥ 95%','Malaysia shipment milestone compliance ≥ 95%','Selisih biaya logistik terhadap approval dalam batas yang disetujui'],
    monthly: ['Analisis fulfillment dan outstanding','Review demand vs supply','Reconcile order, delivery, dan POD','Review performa dan biaya ekspedisi Malaysia','Usulkan perbaikan cycle time'],
    weekly: ['Shortage analysis','Commitment produksi','Delivery plan','Outstanding control','NPD stock readiness','Update shipment Malaysia: forwarder, dokumen, pickup, customs, ETA, dan risiko'],
    daily: ['Validasi PO','Cek stok dan WIP','Update shortage dan ETA','Koordinasi issue shipment atau produksi','Follow-up forwarder/ekspedisi Malaysia bila ada shipment aktif','Update outstanding dan milestone pengiriman']
  },
  admin: {
    purpose: 'Menjamin transaksi, data, dashboard, dokumentasi, dan evidence lengkap, akurat, dan tepat waktu.',
    jobdesk: ['Input dan validasi transaksi Odoo','Memperbarui master data','Memvalidasi laporan seller','Memperbarui dashboard dan action tracker','Mengelola dokumen dan evidence','Mendukung arsip quotation, dokumen shipment, invoice logistik, dan POD Malaysia','Menyiapkan bahan WRM dan MBR'],
...
export default function Home() {
  ...
  const Node = ({id}:{id:string}) => {
    const item = byId[id];
    return <button className="node" style={{background:item.color}} onClick={()=>setSelected(id)}><b>{item.title}</b><small>{item.short}</small></button>;
  };
  ...
  return <main>
    <section className="hero">
      <div className="shell">
        ...
        <div className="badges"><span>6 peran internal</span><span>Hierarchy interaktif</span><span>AI Analyzer</span><span>Bahasa Indonesia</span></div>
      </div>
    </section>
    ...
    <section id="hierarchy" className="section shell">
      ...
      <div className="org">
        <button className="node top" style={{background:byId.supervisor.color}} onClick={()=>setSelected('supervisor')}><b>{byId.supervisor.title}</b><small>{byId.supervisor.short}</small></button>
        <div className="line" />
        <div className="row">
          <div className="branch"><Node id="keyaccount"/><div className="line smallLine"/><Node id="relation"/></div>
          <div className="branch"><Node id="acquisition"/></div>
          <div className="branch"><Node id="operations"/></div>
          <div className="branch"><Node id="admin"/></div>
        </div>
      </div>
    </section>
    ...
    <section id="analyzer" className="section alt">
      <div className="shell">
        ...
        <div className="card inputCard">
          <textarea value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Contoh: Kami akan mengirim kontainer ke Malaysia dan perlu koordinasi dengan forwarder. Siapa PIC-nya dan apa langkahnya?" />
    ...
    <section id="cadence" className="section shell">
      <h2>Team Cadence</h2>
      <div className="resultGrid"><article className="card"><span className="mini">Daily</span><h3>Daily Control</h3><p>Prioritas hari ini, blocker, seller risk, critical PO, shipment Malaysia, pipeline, dan marketplace alert.</p></article><article className="card"><span className="mini">Weekly</span><h3>WRM & Operations Review</h3><p>Target vs actual, seller, PO, shortage, delivery, shipment Malaysia, pipeline, marketplace, dan action tracker.</p></article><article className="card"><span className="mini">Monthly</span><h3>Monthly Business Review</h3><p>Revenue, margin, seller health, fulfillment, logistics Malaysia, conversion acquisition, marketplace P&L, dan KPI individu.</p></article></div>
    </section>
